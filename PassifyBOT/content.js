...
<span style="font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:1px; color:#4f46e5;">🚀 Passify Engine Pro</span>
<button id="passify-close-btn" style="background:none; border:none; cursor:pointer; font-size:18px; color:#94a3b8; font-weight:bold; line-height:1;">×</button>
</div>
    ...
    try {
        ...
        if (filaAlvo === "Pausa 20 / Fim de expediente") {
            ...
            if (filaDetectada) {
                ...
                setLog('⚠️ Não localizado na tela. Pulando ticket...');
                chrome.runtime.sendMessage({ action: "TICKET_PROCESSED", idBanco: idBanco, status: "Ignorado: Não localizado na tela" });
                return;
            }
        }
        ...
        // SEU PONTO DE ERRO TRATADO: Se o botão de transferir não for encontrado, pula estrategicamente o ticket.
        const transferBtn = findTransferButtonDeep();
        if (!transferBtn) {
            setLog(`⚠️ Botão transferir ausente. Pulando ticket...`);
            chrome.runtime.sendMessage({ action: "TICKET_PROCESSED", idBanco: idBanco, status: "Ignorado: Botão transferir ausente" });
            return;
        }
        ...
        if (!queueInput) {
            setLog('⚠️ Erro: Campo de seleção de fila sumiu. Pulando...');
            chrome.runtime.sendMessage({ action: "TICKET_PROCESSED", idBanco: idBanco, status: "Erro: Campo fila ausente" });
            return;
        }
        ...
        // Envia confirmação de Sucesso ao Background
        chrome.runtime.sendMessage({ action: "TICKET_PROCESSED", idBanco: idBanco, status: "Concluído" });

    } catch (err) {
        setLog('❌ Erro na execução da aba: ' + err.message);
        // Garante que mesmo em caso de erro crítico, a fila ande
        chrome.runtime.sendMessage({ action: "TICKET_PROCESSED", idBanco: idBanco, status: "Erro: " + err.message });
    }
}
})();
